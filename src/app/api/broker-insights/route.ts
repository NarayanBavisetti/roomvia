...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!serviceKey) {
      ...
      console.log('Skipping auth check - using service role for broker insights')
    }
    ...
    if (!serviceKey && !session?.user) {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log('Development mode: allowing unauthenticated access with mock data')
        ...
        return NextResponse.json({
          ...
          data: {
            insights: {
              ...
              priceRanges: { '< ₹15k': 0, '₹15k-25k': 0, '₹25k-40k': 0, '> ₹40k': 0 },
              popularAmenities: [],
              locationPreferences: []
            },
...
**To test Gemini integration:** Add \`&test=true\` to the URL`,
    ...
    console.log(`Fetching filter data for ${city} from ${startDate.toISOString()} to ${endDate.toISOString()}`)
    ...
    try {
      const aiPrompt = createInsightPrompt(insights, city, days, filterData?.length === 0)
      console.log('Attempting to generate AI insights with Gemini')
      
      aiResult = await geminiService.generateInsights(session?.user?.id || 'service-role-user', aiPrompt)
      analysisType = filterData?.length === 0 ? 'listing_ai_analysis' : 'filter_ai_analysis'
    } catch (error: unknown) {
      console.log('Gemini failed, using fallback insights:', error instanceof Error ? error.message : 'Unknown error')
      fallbackUsed = true
      
      // Generate fallback insights
      aiResult = generateFallbackInsights(insights, city, days)
      analysisType = filterData?.length === 0 ? 'listing_statistical' : 'filter_statistical'
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        metadata: {
          ...
          note: filterData?.length === 0 ? 'Analysis based on recent listing data' : 'Analysis based on user search filters'
        }
      }
    })
...
async function analyzeListingData(supabase: any, city: string, days: number) {
  ...
  listings.forEach((listing: any) => {
    if (listing.property_type) {
      propertyTypes[listing.property_type] = (propertyTypes[listing.property_type] || 0) + 1
    }
  })
...
function analyzeFilterData(filterData: Array<{
  id: string
  user_id: string
  city?: string
  state?: string
  ...
  filterData.forEach(item => {
    if (item.property_type) {
      propertyTypes[item.property_type] = (propertyTypes[item.property_type] || 0) + (item.usage_count || 1)
    }
  })
  ...
  filterData.forEach(item => {
    if (item.area) {
      locationCount[item.area] = (locationCount[item.area] || 0) + (item.usage_count || 1)
    }
  })
...
function createInsightPrompt(insights: {
...
}, city: string, days: number, isListingBased: boolean = false): string {
...
- Average searches per user: ${insights.uniqueUsers > 0 ? (insights.totalSearches / insights.uniqueUsers).toFixed(1) : 0}

PROPERTY TYPE DEMAND:
${insights.propertyTypeDistribution.map((item) => 
  `- ${item.property_type}: ${item.search_count} searches`
).join('\n')}

PRICE RANGE PREFERENCES:
${Object.entries(insights.priceRanges).map(([range, count]) => 
  `- ${range}: ${count} searches`
).join('\n')}

TOP AMENITIES SEARCHED:
${insights.popularAmenities.slice(0, 8).map((item, index) => 
  `${index + 1}. ${item.amenity}: ${item.usage_count} times`
).join('\n')}

TOP LOCATION PREFERENCES:
${insights.locationPreferences.slice(0, 8).map((item, index) => 
  `${index + 1}. ${item.area}: ${item.search_count} searches`
).join('\n')}
...
function generateFallbackInsights(insights: {
  totalSearches: number
  uniqueUsers: number
  propertyTypeDistribution: Array<{ property_type: string; search_count: number }>
  priceRanges: Record<string, number>
  ...
  const topPropertyType = insights.propertyTypeDistribution[0]?.property_type || 'N/A'
  ...
  const topPriceRange = Object.entries(insights.priceRanges)
    .sort(([,a], [,b]) => b - a)[0]?.[0] || 'N/A'
  
  const avgSearchesPerUser = insights.uniqueUsers > 0 
...
• ${topPropertyType} is the most searched property type with ${insights.propertyTypeDistribution[0]?.search_count || 0} searches
• Total of ${insights.propertyTypeDistribution.length} different property types in demand
• Market shows ${insights.totalSearches > 100 ? 'high' : insights.totalSearches > 50 ? 'moderate' : 'low'} search activity

**Budget Range Preferences:**
• ${topPriceRange} is the most popular budget range
• Price distribution shows ${Object.keys(insights.priceRanges).length} active price segments
• Market accessibility varies across different income groups

## 🗺️ LOCATION INSIGHTS

**Area Demand:**
• ${topLocation} leads with ${insights.locationPreferences[0]?.search_count || 0} searches
• ${insights.locationPreferences.length} different areas showing search activity
• Location preferences indicate ${insights.locationPreferences.length > 10 ? 'diverse' : 'concentrated'} market demand

**Geographic Distribution:**
• Search activity spread across ${insights.locationPreferences.length} locations
• Top 3 areas account for ${insights.locationPreferences.slice(0, 3).reduce((sum, item) => sum + item.search_count, 0)} searches

## 💰 PRICING RECOMMENDATIONS

**Rent Optimization:**
• Focus on ${topPriceRange} segment for maximum market reach
• Consider ${insights.propertyTypeDistribution.slice(0, 2).map(p => p.property_type).join(' and ')} properties for best demand
• Price competitively within popular ranges to attract more inquiries

## 🏠 TENANT PREFERENCES

**Amenity Priorities:**
• ${topAmenity} is the most requested feature (${insights.popularAmenities[0]?.usage_count || 0} searches)
• Top 5 amenities: ${insights.popularAmenities.slice(0, 5).map(a => a.amenity).join(', ')}
...
**2. Inventory Focus:**
• Prioritize ${insights.propertyTypeDistribution.slice(0, 2).map(p => p.property_type).join(' and ')} property types
• Consider expanding to ${insights.locationPreferences.slice(1, 3).map(l => l.area).join(' and ')} for growth opportunities

**3. Marketing Strategy:**
• Highlight ${insights.popularAmenities.slice(0, 3).map(a => a.amenity).join(', ')} in property descriptions
• Focus advertising efforts in ${insights.locationPreferences.slice(0, 3).map(l => l.area).join(', ')}

**4. Competitive Advantage:**
• With ${insights.uniqueUsers} active users and ${avgSearchesPerUser} searches per user, market shows ${insights.totalSearches > 100 ? 'high engagement' : 'growth potential'}
• Focus on quick response times as users are actively searching

---
*Analysis based on ${insights.totalSearches} searches from ${insights.uniqueUsers} unique users over ${days} days in ${city}.*
...
**Note:** This analysis was generated using statistical insights due to temporary AI service limitations. Contact support if you need detailed AI-powered recommendations.`
  ...
  const recommendations = [
    ...
    `Consider expanding inventory to ${insights.locationPreferences[1]?.area || 'emerging areas'} for growth`
  ]

  return {
    content: fallbackContent,
    confidence: 0.75, // Lower confidence for fallback
    recommendations: recommendations.filter(r => !r.includes('undefined') && !r.includes('N/A'))
  }
}
...
async function handleTestDataRequest(city: string, days: number) {
  ...
  try {
    ...
    console.log('Attempting to generate AI insights with Gemini using sample data')
