...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      const aiResult = await geminiService.generateInsights(
        session.user.id,
        aiPrompt
      );
      ...
      const fallbackResult = generateStatisticalInsights(
        marketData,
        city,
        days
      );
...
async function analyzeFilterData(
  filterData: Array<{
    id: string;
    user_id: string;
    city?: string;
  ...
  filterData.forEach((item) => {
    const propertyType =
      item.property_type ||
      extractFromFilters(item.filters, "property_type") ||
      "apartment";
    propertyTypeCount[propertyType] =
      (propertyTypeCount[propertyType] || 0) + (item.usage_count || 1);
  });
  ...
  filterData.forEach((item) => {
    ...
    if (item.filters) {
      const amenities =
        extractArrayFromFilters(item.filters, "amenities") || [];
  ...
  filterData.forEach((item) => {
    ...
    const maxRent = Number(
      item.max_rent || maxRentValue || item.min_rent || 25000
    );
  ...
  filterData.forEach((item) => {
    const area =
      item.area ||
      extractFromFilters(item.filters, "locality") ||
      extractArrayFromFilters(item.filters, "locality")?.[0];
    if (area) {
  ...
  filterData.forEach((item) => {
    ...
    // Gender preferences
    const gender =
      extractFromFilters(item.filters, "gender") ||
      extractArrayFromFilters(item.filters, "gender")?.[0];
    if (gender) {
      totalGender += count;
    ...
    // Broker preferences
    const brokerPref =
      extractFromFilters(item.filters, "broker") ||
      extractArrayFromFilters(item.filters, "broker")?.[0];
    if (brokerPref) {
  ...
  const demographics = {
    malePercentage:
      totalGender > 0 ? Math.round((maleCount / totalGender) * 100) : 0,
    femalePercentage:
      totalGender > 0 ? Math.round((femaleCount / totalGender) * 100) : 0,
    brokerPreference:
      totalBrokerPref > 0
        ? Math.round((brokerCount / totalBrokerPref) * 100)
        : 0,
    noBrokerPreference:
      totalBrokerPref > 0
        ? Math.round((noBrokerCount / totalBrokerPref) * 100)
        : 0,
  };
  ...
  filterData.forEach((item) => {
    const occupancy =
      extractFromFilters(item.filters, "occupancy") ||
      extractArrayFromFilters(item.filters, "occupancy")?.[0];
    if (occupancy) {
      occupancyCount[occupancy] =
        (occupancyCount[occupancy] || 0) + (item.usage_count || 1);
    }
  });
  ...
  filterData.forEach((item) => {
    const lifestyle =
      extractFromFilters(item.filters, "lifestyle") ||
      extractArrayFromFilters(item.filters, "lifestyle")?.[0];
    const food =
      extractFromFilters(item.filters, "food") ||
      extractArrayFromFilters(item.filters, "food")?.[0];

    if (lifestyle) {
      lifestyleCount[lifestyle] =
        (lifestyleCount[lifestyle] || 0) + (item.usage_count || 1);
    }
    if (food) {
      lifestyleCount[food] =
        (lifestyleCount[food] || 0) + (item.usage_count || 1);
    }
  });
...
function extractArrayFromFilters(
  filters: Record<string, unknown> | null | undefined,
  key: string
): string[] | null {
  if (!filters || typeof filters !== "object") return null;
...
function createComprehensiveInsightPrompt(
  data: MarketInsightsData,
  city: string,
  days: number
): string {
...
${data.propertyTypeDemand
  .map(
    (item) =>
      `- ${item.property_type}: ${item.search_count} searches (${item.percentage}%)`
  )
  .join("\n")}
...
TOP AMENITIES:
${data.amenitiesRanking
  .slice(0, 8)
  .map((item, i) => `${i + 1}. ${item.amenity}: ${item.usage_count} searches`)
  .join("\n")}

POPULAR AREAS:
${data.popularAreas
  .slice(0, 8)
  .map((item, i) => `${i + 1}. ${item.area}: ${item.search_count} searches`)
  .join("\n")}
...
OCCUPANCY PATTERNS:
${data.occupancyPreferences
  .slice(0, 5)
  .map((item) => `- ${item.occupancy}: ${item.count} searches`)
  .join("\n")}

LIFESTYLE PREFERENCES:
${data.lifestylePreferences
  .slice(0, 5)
  .map((item) => `- ${item.lifestyle}: ${item.count} searches`)
  .join("\n")}
...
function generateStatisticalInsights(
  ...
  const topBudgetRange =
    Object.entries(data.budgetDistribution).sort(
      ([, a], [, b]) => b - a
    )[0]?.[0] || "N/A";
...
The ${city} rental market shows ${
    data.totalSearches > 500
      ? "strong"
      : data.totalSearches > 100
      ? "moderate"
      : "emerging"
  } activity with ${data.totalSearches} total searches from ${
    data.uniqueUsers
  } unique users over ${days} days.

**Property Type Trends:**
- ${topProperty} dominates demand with ${
    data.propertyTypeDemand[0]?.percentage
  }% market share
- ${data.propertyTypeDemand.length} different property types show active demand
- Market concentration indicates ${
    data.propertyTypeDemand[0]?.percentage > 50 ? "focused" : "diverse"
  } tenant preferences
...
- ${
    data.budgetDistribution["> ₹40k"] > data.budgetDistribution["< ₹15k"]
      ? "Premium"
      : "Affordable"
  } segment dominance
...
- ${topArea} shows highest search volume (${
    data.popularAreas[0]?.search_count
  } searches)
- Top 5 areas account for ${data.popularAreas
    .slice(0, 5)
    .reduce((sum, area) => sum + area.search_count, 0)} total searches
- Geographic spread across ${data.popularAreas.length} distinct locations

## 👥 Tenant Demographics
**Target Segments:**
- Gender preference: ${data.demographics.malePercentage}% male, ${
    data.demographics.femalePercentage
  }% female
- Broker relationship: ${
    data.demographics.noBrokerPreference
  }% prefer direct deals
- Lifestyle preferences favor ${
    data.lifestylePreferences[0]?.lifestyle || "standard"
  } living
...
${data.amenitiesRanking
  .slice(0, 5)
  .map((a) => `- ${a.amenity} (${a.usage_count} requests)`)
  .join("\n")}

**Occupancy Patterns:**
${data.occupancyPreferences
  .slice(0, 3)
  .map((o) => `- ${o.occupancy}: ${o.count} preferences`)
  .join("\n")}`;
  ...
  const recommendations = [
    ...
    `Target ${
      data.demographics.noBrokerPreference > 50
        ? "direct-to-tenant"
        : "broker-assisted"
    } marketing approach`,
    `Consider ${
      data.propertyTypeDemand[1]?.property_type || "alternative"
    } properties for portfolio diversification`,
    `Focus on ${
      data.occupancyPreferences[0]?.occupancy || "flexible"
