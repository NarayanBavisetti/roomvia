...
class AnalyticsService {
  ...
  async trackEvent(
    eventType: 'click' | 'scroll' | 'save' | 'inquiry' | 'phone_reveal' | 'share',
    listingId?: string,
  ...
  async getBrokerAnalytics(brokerId: string, daysBack: number = 30): Promise<any> {
    try {
  ...
  async getListingAnalytics(brokerId: string, daysBack: number = 30): Promise<any[]> {
    try {
  ...
  // Get search trends
  async getSearchTrends(daysBack: number = 30): Promise<{
    keywords: string[]
    locations: string[]
    ...
    try {
      ...
      const trends = data?.reduce((acc: any, search: any) => {
  ...
  private groupPriceRanges(ranges: { min: number; max: number }[]): { min: number; max: number; count: number }[] {
  ...
  async getBrokerInsights(brokerId: string, forceRefresh: boolean = false): Promise<any> {
    try {
      if (!forceRefresh) {
        // Check cache first
