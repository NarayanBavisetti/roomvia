...
export function useAnalyticsIntegration() {
  ...
  useEffect(() => {
    // Dynamic import to avoid require() and keep client-only usage safe
    const update = async () => {
      if (typeof window !== "undefined") {
        const mod = await import("@/lib/analytics");
        mod.analyticsService.updateUserSession(user?.id);
      }
    };
    update();
  }, [user]);
...
export function useListingAnalytics() {
  ...
  const onListingInquiry = (
    listingId: string,
    inquiryType: "message" | "phone" | "email" = "message"
  ) => {
...
export function useSearchAnalytics() {
  ...
  const onSearch = (
    query?: string,
    filters?: import("@/lib/analytics").SearchFilters,
    resultsCount?: number
  ) => {
